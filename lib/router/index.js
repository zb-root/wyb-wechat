  ...
  service.onReady().then(() => {
    ...
    router.use('/authuri', function (req, res) {
      ...
      if(url){
        ...
        surl = 'http://' + service.config.domain + '/wechat_oauth.html?redirect_uri=' + encodeURIComponent(redirect_uri)
      }
      let str = service.oauth.getAuthorizeURL(surl, '', 'snsapi_userinfo')
      res.send({uri: str})
    })
